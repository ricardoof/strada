...
import { CreditCard as CardIcon, ArrowLeft, Lock, Check, ChevronDown } from "lucide-react";
...
export function CreditCard() {
    ...
    const [cardData, setCardData] = useState({
        ...
        installments: '1' // Novo estado para parcelas
    });
        ...
        try {
            const res = await fetch('http://localhost:3333/checkout', {
                ...
                body: JSON.stringify({
                    ...
                    pagamento: {
                        metodo: 'credit_card',
                        parcelas: cardData.installments // Envia o nº de parcelas (opcional para seu backend atual)
                    }
                })
            });
            ...
            <div className="w-full max-w-2xl">
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 animate-in zoom-in duration-300">
                    <div className="text-center mb-8">
                        <h2 className="text-2xl font-bold text-gray-800">Pagamento com Crédito</h2>
                        <p className="text-gray-500">Cartão de Crédito</p>
                    </div>

                    {/* --- CARTÃO VISUAL LARANJA --- */}
                    <div className="mb-10 perspective-1000">
                        <div className="bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl p-8 text-white shadow-2xl transform hover:scale-105 transition-all duration-500 relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-12">
                                <div className="text-sm opacity-80 font-medium tracking-widest uppercase">Strada Card</div>
                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-orange-500 focus:outline-none text-lg bg-white appearance-none transition-colors cursor-pointer"
                                >
                                    {Array.from({ length: 10 }, (_, i) => i + 1).map((num) => (
                                        <option key={num} value={num}>
                                            {num}x de R$ {(valores.total / num).toFixed(2).replace('.', ',')} {num === 1 ? '(à vista)' : 'sem juros'}
                                        </option>
                                    ))}
                                </select>
                        ...
                        <div className="pt-6 border-t border-gray-100 mt-6">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className={`w-full bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white font-bold py-4 rounded-xl shadow-lg transform active:scale-95 transition-all duration-200 flex items-center justify-center gap-2 ${loading ? 'opacity-70 cursor-wait' : ''}`}
                            >
                                {loading ? 'Processando...' : <>Confirmar Pagamento <Check size={20} /></>}
                            </button>
